...
!(async () => {
	...
	else
	{
		...
		for (numUser = 0; numUser < totalUser; numUser++)
		{
			...
			//await appShareTaskReq("task_51_1111")

			if(TxStockAppUrl && TxStockAppHeader)
			{
				await userhome(); //第二次金币查询
				coinEnd = coinInfo
				rewardCoin = coinEnd - coinStart;
				await orderQuery()
				
				$.log(`账号：${nickname}，本次运行获得${rewardCoin}金币\n`)
			}
		...
		for (numUser = 0; numUser < totalUser; numUser++)
		{
			...
			for (helpUser = 0; helpUser < totalUser; helpUser++){
				if(helpUser != numUser) {
					await bullShareCode(bullInviteCodeArr[helpUser],bullHelpCodeArr[helpUser])
				}
			}
		}
...
function orderQuery() {
  return new Promise((resolve, reject) => {
    ...
    $.get(testurl, async (error, resp, data) => {
	  ...
	  if(result.retcode == 0){
		  nickname = result.shop_asset.nickname
		  if(cash != 0){
...
//长牛互助
async function bullShareCode(inviteCode="",helpCode="") {
  return new Promise((resolve) => {
	shareDate = inviteCode ? signday : ""
    let signurl = {
      url: `https://zqact03.tenpay.com/cgi-bin/activity_year_party.fcgi?invite_code=${inviteCode}&help_code=${helpCode}&share_date=${shareDate}&type=bullish&action=home&actid=1105&_=${rndtime}&openid=${app_openid}&fskey=${app_fskey}&channel=1&access_token=${app_token}&_appName=${app_appName}&_appver=${app_appver}&_osVer=${app_osVer}&_devId=${app_devId}`,
      headers: {
        'Cookie': app_ck,
        'Accept': `*/*`,
        'Connection': `keep-alive`,
...
//分享任务-微信端完成
function appShareTaskDone(share_type,share_code) {
  return new Promise((resolve, reject) => {
    let url = {
